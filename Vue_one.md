...
```
至此，效果已经全部实现，路子比较野。然后我兴致勃勃去机器上调试，发现不兼容。
内心草泥马一万匹跑过去了，随便说一句。使用的机器和银行的那种自助查询机器类似，平台是谷歌浏览器，强制全屏下无效，于是只能使用原生来解决这个方法，下面附带原生的方法，另外附上BS的API，有兴趣的小伙伴可以去研究一下，BS应该比较适合移动端使用。
